...
class LuckyFollowController extends AddonsController
{
    ...
    public function lists()
    {
        ...
        if ($search) {
            ...
            } elseif (! empty($truename_follow_ids)) {
                $map['follow_id'] = array(
                    'exp',
        ...
        if (! empty($type) && ($flag == 0 || $flag == 1)) {
            ...
            if ($type == 'award') {
                $qulist = M('lucky_follow')->join('left join ' . $fix . 'sport_award on ' . $fix . 'lucky_follow.award_id=' . $fix . 'sport_award.id')
                    ->where(array(
                ...
                $qulist = M('lucky_follow')->join('left join ' . $fix . 'follow on ' . $fix . 'lucky_follow.follow_id=' . $fix . 'follow.id ')
                    ->where(array(
                    'uid' => $uid
                ))
                    ->field($fix . "lucky_follow.id")
    ...
    private function getAwardNames($op)
    {
        if (! empty($op)) {
            ...
            $awardName = M('sport_award')->where($map1)
                ->field('id,name')
                ->select();
    ...
    public function luckyFollowDetail()
    {
        ...
        $support_team = M('sports_support')->where($map)
        ...
        $drumCountData = M('sports_drum')->where($map2)
        ...
        // 获取抽奖数
        $drawCountData = M('draw_follow_log')->where($map2)
        ...
        $joinCountData = M('join_count')->where($map3)
            ->field('aim_id,count')
            ->select();
        if ($joinCountData) {
