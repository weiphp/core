...
class WapController extends AddonsController
{
    ...
    public function ajaxCheckCount()
    {
        $gameId = I('games_id', 0, 'intval');
    ...
    public function save_zjInfo($gameId, $awardId, $uid, $awardInfo = array())
    {
        ...
        switch ($awardInfo['award_type']) {
            ...
            case 4:
                ...
                $cardMember = M('card_member')->where($map1)
                    ->field('id,recharge')
                    ->find();
                if ($cardMember) {
                    // 直接加入会员卡
    ...
    // 获取奖品id
    public function _do_lottery($event_id)
    {
        // 奖品列表
        ...
        if ($drawCount <= 0 && empty($d)) {
            $prizeid = 0;
        } elseif ($drawCount <= 0 && $awardNums > $drawCount) {
            // 最大抽奖次数用完，直接返回奖品
    ...
    // $drawCount： 剩余最大抽奖次数
    public function lottery($prizeArr, $drawCount)
    {
        $prize = 0;
    ...
    // 奖品排序
    public function prize_sort($prizeArr)
    {
        $count = 0;
