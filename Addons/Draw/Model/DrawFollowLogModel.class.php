...
class DrawFollowLogModel extends Model
{
    ...
    // 每人总共抽奖次数
    // $time 有值时：每人每天抽奖次数
    public function get_user_attend_count($gamesId, $uid = 0, $time = 0)
    {
        ...
        $data = $this->where($map)
            ->field('sum(count) totals')
            ->select();
        return intval($data[0]['totals']);
    }

    public function hasDraw($sports_id, $follow_id, $update = false)
    {
    ...
    // 获取靓妆用户当日抽奖数
    public function hasDrawByDay($sports_id, $follow_id, $update = false)
    {
        ...
        $info = $this->field('sum(`count`) as num')
            ->where("uid=$user_id and sports_id=$sports_id and follow_id=$follow_id and cTime >= '$cur_date'")
            ->select();
    ...
    public function updateCount($sports_id, $follow_id, $count)
    {
        $count ++;
        $map['sports_id'] = $sports_id;
        $map['follow_id'] = $follow_id;
