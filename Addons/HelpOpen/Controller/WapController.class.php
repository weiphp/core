...
class WapController extends AddonsController
{
    ...
    public function do_help()
    {
        ...
        // 获取奖品列表
        $list = M('help_open_prize')->where($data1)
            ->order('sort asc')
            ->select();
            ...
            if ($k == 0) {
                ...
                if ($vv['prize_type'] == 1) { // 优惠券 OPENTM200474379 优惠券领取成功通知
                    ...
                    if ($couponinfo['collect_count'] >= $couponinfo['num']) {
                        $flat = false;
                    } elseif (! empty($couponinfo['start_time']) && $couponinfo['start_time'] > NOW_TIME) {
                        $flat = false;
                    } elseif (! empty($couponinfo['end_time']) && $couponinfo['end_time'] < NOW_TIME) {
                        $flat = false;
                    }
                    ...
                    if ($scouponinfo['collect_count'] >= $scouponinfo['num']) {
                        $flat = false;
                    } elseif (! empty($scouponinfo['start_time']) && $scouponinfo['start_time'] > NOW_TIME) {
                        $flat = false;
                    } elseif (! empty($scouponinfo['end_time']) && $scouponinfo['end_time'] < NOW_TIME) {
                        $flat = false;
                    }
    ...
    public function _sendPrize($help_id, $uid, $prize)
    {
    ...
    public function my_prize()
    {
        ...
        // 获奖人数
        $where = "sn_id>0";
        $total = D('HelpOpenUser')->where($map)
            ->where($where)
            ->count();
    ...
    public function _time($start_time = '', $end_time = '')
    {
    ...
    public function prize_log()
    {
        ...
        $this->assign('info', $info);
        // 读取模型数据列表
        $data = M('help_open_user')->field(true)
            ->where($map)
            ->where($where)
            ->order('id desc')
            ->page($page, 20)
        ...
        /* 查询记录总数 */
        $count = M('help_open_user')->where($map)
            ->where($where)
            ->count();
        $list_data['list_data'] = $data;
    ...
    public function error_tips($info)
    {
        $msg = '';
        if ($info['status'] == 0) {
            $msg = '活动已经禁用';
