...
class HelpOpenController extends AddonsController
{
    ...
    public function _initialize()
    {
        ...
        $res['title'] = '模板消息配置';
        $res['url'] = addons_url('HelpOpen://HelpOpen/config', array(
            'mdm' => $_GET['mdm']
        ));
    ...
    public function lists()
    {
        ...
        foreach ($data as &$vo) {
            ...
            $join_list = $dao->where($user_map)
                ->field('invite_uid,sn_id,join_count')
                ->select();
    ...
    public function _prize_list($help_id = 0)
    {
        if ($help_id) {
            $map['help_id'] = $help_id;
            $map['is_del'] = 0;
            $list = M('help_open_prize')->where($map)
                ->order('sort asc')
                ->select();
    ...
    // 保存优惠信息
    public function setPrize($help_id, $data)
    {
    ...
    public function share_lists()
    {
        ...
        $this->assign('search_url', U('share_lists', array(
            'id' => $hmap['id'],
            'mdm' => $_GET['mdm']
        )));
        $this->display('lists');
    ...
    public function checkDate()
    {
        ...
        if (! I('post.start_time')) {
            $this->error('请选择开始时间');
        } elseif (! I('post.end_time')) {
            $this->error('请选择结束时间');
        } elseif (strtotime(I('post.start_time')) > strtotime(I('post.end_time'))) {
