...
class AskQuestionModel extends Model
{
    ...
    public function getQuestionsByAskid($ask_id, $question_id = '', $update = false)
    {
        $key = 'getQuestionsByAskid_' . $ask_id;
        $list = S($key);
        if ($list === false || $update) {
            $map['ask_id'] = intval($ask_id);
            $list2 = $this->where($map)
    ...
    public function clear($ids, $type = '', $uid = '')
    {
        if (empty($ids)) {
            return false;
        }
    ...
    public function setLastQuestion($ask_id)
    {
        $map['ask_id'] = $ask_id;
        $this->where($map)->setField('is_last', 0);
        
        $map2['id'] = $this->where($map)
    ...
    public function getQuestionTitle($id, $ask_id)
    {
    ...
    public function getQuestionAnswer($id, $ask_id)
    {
    ...
    public function getQuestionAnswerExtra($id, $ask_id, $answer)
    {
