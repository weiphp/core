...
class WapController extends BaseController
{
    ...
    public function index()
    {
        ...
        } elseif ($info['number'] && ! $info['phone'] && $info['status'] == 2) {
    ...
    // 活动赠送
    public function _send_reward($member_id, $table, $credit_type, $recharge)
    {
    ...
    public function _do_card_reward()
    {
        // 开卡即送活动
        $map['start_time'] = array(
            'lt',
        ...
        $map['token'] = get_token();
        $event_info = M('card_reward')->where($map)
            ->order('id desc')
            ->find();
        if ($event_info) {
    ...
    public function do_bind_card()
    {
        ...
        $map['token'] = get_token();
        $map['is_get'] = 1;
        $shopMember = M('shop_card_member')->where($map)
            ->order('id desc')
            ->find();
        // $sql= M ()->getLastSql () ;
        if ($shopMember) {
            ...
            if ($info) {
                ...
                $map1['token'] = get_token();
                $event_info = M('card_reward')->where($map1)
                    ->order('id desc')
                    ->find();
                if ($event_info) {
                    if ($event_info['type'] == 0) { // 送积分
    ...
    public function score()
    {
        ...
        if ($isERP) {
            ...
            $get_data = M('credit_data')->where($map1)
                ->field("from_unixtime(cTime,'%Y-%m-%d') allday,credit_name,score,credit_title")
                ->order('id desc')
                ->select();
            ...
            foreach ($get_data as &$vo) {
                ...
                } elseif (! $creditTitle[$vo['credit_name']]) {
    ...
    public function recharge()
    {
        ...
        if ($card_id) {
            ...
            $rechargeTotal = M('recharge_log')->where($map1)
                ->field("sum(recharge) total_recharge")
                ->select();
            ...
            $delRecharge = M('recharge_log')->where($map2)
                ->field("sum(recharge) total_recharge")
                ->select();
            
            $buyTotal = M('buy_log')->where($map3)
                ->field("sum(pay) total_recharge")
                ->select();
            ...
            $rechargeData = M('recharge_log')->where($map1)
                ->field("from_unixtime(cTime,'%Y-%m-%d') allday,recharge,remark")
                ->order('id desc')
                ->select();
            
            // 扣除
            $delRechargeData = M('recharge_log')->where($map2)
                ->field("from_unixtime(cTime,'%Y-%m-%d') allday,recharge,remark")
                ->select();
            // 消费
            $buyData = M('buy_log')->where($map3)
                ->field("from_unixtime(cTime,'%Y-%m-%d') allday,pay")
                ->select();
    ...
    public function do_signin()
    {
        ...
        if ($is_can == 1) {
            ...
            if (stripos('..' . $config['reply'], '[排行榜]') >= 1) {
                ...
                if ($top5) {
                    ...
                    if (! empty($uids)) {
                        ...
                        $w2['token'] = get_token();
                        $members = M('public_follow')->where($w2)
                            ->field('uid,openid')
                            ->select();
            ...
            if ($config['continue_day'] == 1) {
                ...
                $amap['uid'] = $this->mid;
                $logdata = M('signin_log')->where($amap)
                    ->order('sTime desc')
                    ->limit($config['continue_day'])
                    ->getfields('sTime');
                $daycount = 0;
                $days = 3600 * 24;
    ...
    public function score_exchange()
    {
        ...
        $dataList = M('card_score')->where($map)
            ->order('id desc')
            ->select();
        ...
        foreach ($data as &$v) {
            ...
            if ($info['collect_count'] >= $info['num']) {
                $nocount = 1;
            } elseif (! empty($info['start_time']) && $info['start_time'] > NOW_TIME) {
                $nocount = 1;
    ...
    public function do_score_exchange()
    {
        $id = I('get.id');
    ...
    // 每日推荐
    public function day_recommend()
    {
        $this->display();
    }
    // 生日特权
    public function custom_privilege()
    {
    ...
    public function check_ERP_member()
    {
        $mobile = I('phone');
        $managerId = session('manager_id');
        $userInfo = get_userinfo($managerId);
    ...
    public function get_privilege()
    {
        ...
        if (empty($msg)) {
            $save['is_send'] = 1;
            $rr = M('card_custom_log')->where(array(
                'id' => $id
            ))->save($save);
            if (! $rr) {
                $msg = '领取失败!';
