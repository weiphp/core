...
class CardModel extends Model
{
    ...
    public $tableName = 'card_member';

    public function addMoney($uid, $money, $log)
    {
        $map['uid'] = $uid;
        $map['token'] = get_token();
    ...
    // 关注初始化会员
    public function init_card_member($openid = '', $uid = 0)
    {
        $token = get_token();
        if ($openid) {
            $dataMap['token'] = $token;
    ...
    public function _do_card_reward($uid)
    {
        // 开卡即送活动
    ...
    // 同步ERP会员信息
    public function updateERPMember($cardInfo, $uid = 0, $openid = '', $levelInfo)
    {
        $msg = '';
        // 对接erp
        $managerId = session('manager_id');
        ...
        if ($userInfo['secretID'] && $userInfo['secretKey']) {
            ...
            } elseif ($member['Rows'][0]['状态'] == 1 || $member2['Rows'][0]['状态'] != 0) {
    ...
    public function addERPMember($cardInfo, $uid = 0, $openid = '')
    {
        $msg = '';
        // 对接erp
        $managerId = session('manager_id');
    ...
    // 同步积分
    public function synchro_score($openid, $total_score)
    {
