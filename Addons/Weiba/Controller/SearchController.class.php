...
class SearchController extends AddonsController
{
 ...
 // 周
    ...
    public function person()
    {
        if ($this->keywd != "") {
            ...
            foreach ($list['data'] as $key => $value) {
                ...
                $list['data'][$key] = array_merge($list['data'][$key], array(
                    'feedInfo' => $lastFeedData[$value['uid']]
                ));
                $list['data'][$key] = array_merge($list['data'][$key], array(
                    'followState' => $followState[$value['uid']]
                ));
                $list['data'][$key] = array_merge($list['data'][$key], array(
                    'remark' => $remarkInfo[$value['uid']]
                ));
            }
            $this->assign('searchResult', $list);
            // dump($list);
    ...
    private function _getWeibaName($weiba_ids)
    {
        ...
        $names = M('weiba')->where($map)
            ->field('weiba_id,weiba_name')
            ->findAll();
        foreach ($names as $n) {
    ...
    {
        $user_tag = model('Tag')->setAppName('User')
            ->setAppTable('user')
            ->getAppTags($uids);
