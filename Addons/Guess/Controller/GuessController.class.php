...
class GuessController extends AddonsController
{
    ...
    public function edit()
    {
        ...
        if (IS_POST) {
            ...
            $option_list = M('guess_option')->where('guess_id=' . $id)
                ->order('`order` asc')
                ->select();
    ...
    public function _getGuessInfo($id)
    {
        // 检查ID是否合法
        if (empty($id) || 0 == $id) {
            $this->error("错误的竞猜ID");
    ...
    public function common_log($guess_id, $map, $title = '竞猜记录')
    {
        // $guess_id=I('guess_id');
        $nav[0]['title'] = "竞猜";
        $nav[0]['class'] = "";
