...
class GuessOptionModel extends Model
{
    ...
    public function set($guess_id, $post)
    {
    ...
    public function getGuessOption($guess_id, $update = false, $data = array())
    {
        $key = 'GuessOption_getGuessOption_' . $guess_id;
        $info = S($key);
        if ($info === false || $update) {
            $map['guess_id'] = $guess_id;
            $info = (array) (empty($data) ? $this->where($map)
                ->order('`order` asc')
                ->select() : $data);
    ...
    // 投票选项信息的num+1
    public function updateOptCount($guess_id, $ids)
    {
