...
class Update
{
    ...
    public function downloadFile($packageURL)
    {
    ...
    public function unzipPackage($packageName, $targetDir = '', $cleanFile = true)
    {
        if (empty($targetDir)) {
     ...
     */
    //
    public function zipPackage($package, $zipDir, $zipName, $removePath = '')
    {
        if (! is_dir($zipDir)) {
    ...
    // 递归覆盖代码文件
    public function overWrittenFile($destination = '', $source = '', $res = array())
    {
        if (empty($source)) {
