...
class WapController extends AddonsController
{
    ...
    public function detail($temp = '', $is_perview = false)
    {
        if (! isset($_GET['uid'])) {
    ...
    // 获取栏目信息
    public function _getBusinessCardColunm($uid)
    {
        $map['uid'] = $uid;
        ...
        $businessCardColunm = M('business_card_column')->where($map)
            ->order('sort asc')
            ->select();
    ...
    {
        $map['from_uid'] = $this->mid;
        $list = M('business_card_collect')->where($map)
            ->order('cTime desc')
            ->selectPage();
    ...
    // 获取目录下的所有模板
    public function _getTemplateByDir($type = 'TemplateDetail', $default = "default")
    {
