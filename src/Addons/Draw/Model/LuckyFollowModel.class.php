...
class LuckyFollowModel extends Model
{
     ...
     */
    public function getGamesLuckyLists($gamesId, $uid = 0, $state = '-1', $aim_table = 'lottery_games')
    {
        $map['token'] = get_token();
    ...
    public function getUserAward($id, $update = false, $data = array())
    {
        ...
        if ($info === false || $update) {
            ...
            $award = D('Addons://Draw/Award')->where($map1)
                ->field('name,img')
                ->find();
    ...
    // 判断当天最多中奖人数
    public function get_day_winners_count($gamesId, $time)
    {
        $map['draw_id'] = $gamesId;
        $map['token'] = get_token();
        ...
        $map['zjtime'] = array(
            'egt',
            strtotime(time_format($time, 'Y-m-d'))
        );
        $data = $this->where($map)
            ->field('count(distinct follow_id) num')
            ->select();
        return $data[0]['num'];
    }
    // 每人总共中奖次数
    // $time 有值时：每人每天中奖次数
    public function get_user_win_count($gamesId, $uid, $time = 0)
    {
        $map['draw_id'] = $gamesId;
        $map['token'] = get_token();
        $map['aim_table'] = 'lottery_games';
        ...
        $data = $this->where($map)
            ->field('sum(num) totals')
            ->select();
        return intval($data[0]['totals']);
    }
    // 获取各抽奖活动对应奖品已发放的数量
    public function getLzwgAwardNum($event_id, $token = '', $aim_talbe = 'lottery_games')
    {
        if ($token && $aim_talbe) {
    ...
    // 根据奖品id，获取中奖者列表
    public function getlistByAwardId($awardid, $update = false)
    {
    ...
    public function getLuckyFollow($sports_id, $award_id, $follow_id, $update = false)
    {
    ...
    public function delayAddData($data, $delay = 10)
    {
    ...
    // 获取各活动场次对应奖品已发放的数量
    public function getAwardNum($sport_id)
    {
    ...
    // 获取粉丝在每场活动的擂鼓数
    public function getDrumCount($follow_id)
    {
    ...
    // 获取粉丝每场活动中奖数
    public function getUserPrizeData($sport_id, $follow_id, $update = false)
    {
        ...
        if ($info === false || $update) {
            ...
            $info = M('lucky_follow')->where($map)
                ->field('award_id,state')
                ->select();
    ...
    // ///////////靓妆////////////////
    public function getLzwgLuckyFollowInfo($id, $update = false, $data = array())
    {
    ...
    public function getLzwgUserPrizeData($lzwg_id, $follow_id, $update = false)
    {
    ...
    // 获取靓装用户所有的中奖
    public function getLzwgUserAllPrizeData($follow_id, $update = false)
    {
