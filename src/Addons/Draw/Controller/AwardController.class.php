...
class AwardController extends AddonsController
{
    ...
    public function export($model = null)
    {
    ...
    public function _coupon($awardType = 2)
    {
        $map['end_time'] = array(
            'gt',
            NOW_TIME
        ...
        if ($awardType == 2) {
            // 优惠券
            $list = M('coupon')->where($map)
                ->field('id,title')
                ->order('id desc')
                ->select();
        } elseif ($awardType == 3) {
            // 代金券
            $list = M('shop_coupon')->where($map)
                ->field('id,title')
                ->order('id desc')
                ->select();
        }
