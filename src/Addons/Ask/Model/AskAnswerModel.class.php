...
class AskAnswerModel extends Model
{
    ...
    public $cache_prefix = 'AskAnswer_';

    public function myLastAnswer($ask_id, $uid = 0, $update = false, $cache_data = array())
    {
        ...
        if ($info === false || $update) {
            $map['ask_id'] = $ask_id;
            $info = empty($cache_data) ? $this->where($map)
                ->order('cTime desc')
                ->find() : $cache_data;
            // lastsql();
            // dump($info);
    ...
    public function hasAnswer($uid, $question_id, $times = 0, $cache_id = 0)
    {
