...
class UserCenterController extends AddonsController
{
    ...
    public function lists()
    {
        ...
        /* 查询记录总数 */
        $count = M()->table($px . 'public_follow as f')
            ->join($px . 'user as u ON f.uid=u.uid')
            ->where($map)
            ->count();
    ...
    // 预先同步好用户组数据
    public function syc_auth_group()
    {
    ...
    public function syc_user()
    {
        ...
        $map['syc_status'] = 0;
        $map['has_subscribe'] = 1;
        $list = M('public_follow')->where($map)
            ->field('uid,openid')
            ->limit(100)
            ->select();
    ...
    // 第三步：同步用户组信息
    public function syc_user_group()
    {
        ...
        $uids = M('public_follow')->where($map)
            ->limit(100)
            ->getFields('uid');
        ...
        $groups = M('auth_group')->where($auth_map)
    ...
    public function clear_score()
    {
        ...
        // 会员卡设置等级
        $firstlevel = M('card_level')->where($map)
            ->order('score asc')
            ->getField('id');
        // 会员卡等级都设为体验卡
        $cardMap1['uid'] = array(
