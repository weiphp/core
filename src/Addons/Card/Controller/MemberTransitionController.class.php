...
class MemberTransitionController extends BaseController
{
    ...
    public function score_lists()
    {
        ...
        if ($search) {
            ...
            } elseif (! empty($mobile_follow_ids)) {
                $map['uid'] = array(
                    'exp',
    ...
    public function buy_tongji()
    {
        ...
        // 本月总消费金额
        $totalPay = M('buy_log')->where($map)
            ->field("sum(pay) totalPay")
            ->select();
        $totalPay = round(floatval($totalPay[0]['totalPay']), 2);
        $this->assign('total_pay', $totalPay);
        
        $data = M('buy_log')->where($map)
            ->field("sum(pay) totalPay,from_unixtime(cTime,'%m-%d') allday")
            ->group("allday")
            ->select();
        foreach ($data as $v) {
            $allDay[] = $v['allday'];
    ...
    public function score_tongji()
    {
        ...
        $getTotal = M('credit_data')->where($map1)
            ->field("sum(score) totalScore")
            ->select();
        // 本月使用积分
        $map2['score'] = array(
            'lt',
        ...
        $useTotal = M('credit_data')->where($map2)
            ->field("sum(score) totalScore")
            ->select();
        ...
        $get_data = M('credit_data')->where($map1)
            ->field("sum(score) totalScore,from_unixtime(cTime,'%m-%d') allday")
            ->group("allday")
            ->select();
        $use_data = M('credit_data')->where($map2)
            ->field("sum(score) totalScore,from_unixtime(cTime,'%m-%d') allday")
            ->group("allday")
            ->select();
