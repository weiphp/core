...
class MemberController extends BaseController
{
    ...
    public function checkMemberId()
    {
        ...
        } elseif (empty($_POST['card_id']) && empty($_POST['number'])) {
            $this->error('请输入会员卡号');
        } elseif ($_POST['number']) {
    ...
    public function checkMemberByAjax()
    {
        $type = I('type', 0);
        $map['number'] = I('number');
        $map['token'] = get_token();
        ...
        if ($info) {
            ...
            if ($type == 0) { // 获取充值赠送活动信息
                $event_info = M('card_recharge')->where($map)
                    ->order('id desc')
                    ->find();
    ...
    public function getSnCode()
    {
        ...
        $map['can_use'] = 1;
        $map['token'] = get_token();
        $snCode = M('sn_code')->where($map)
            ->field('id,prize_title')
            ->select();
    ...
    public function export_member()
    {
        ...
        $list_data = M('card_member')->where($map)
            ->field($fieldStr)
            ->select();
        // dump($list_data);exit;
    ...
    // 活动赠送
    public function _send_reward($event_id, $member_id, $table, $credit_type)
    {
        if (empty($event_id)) {
            return false;
        }
