...
class WapController extends AddonsController
{
    ...
    public function show_error($error, $info = '')
    {
    ...
    public function _detail($my_count = false)
    {
        $id = I('id', 0, 'intval');
    ...
    public function _get_error($data, $my_count = false)
    {
        $error = '';
    ...
    public function sn_set()
    {
        ...
        if ($res) {
            if ($data['addon'] == 'Coupon') {
                ...
                $info = M('sn_code')->where($map2)
                    ->field('sum(is_use) as use_count,count(id) as num')
                    ->find();
