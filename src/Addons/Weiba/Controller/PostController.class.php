...
class PostController extends AddonsController
{
    ...
    public function indexLists()
    {
        ...
        $cateArr = M('weiba')->where($map)->getFields('id,weiba_name');
        session('common_condition', array(
            'is_index' => 1
        ));
        $list_data = $this->_get_model_list($this->model);
        foreach ($list_data['list_data'] as &$data) {
    ...
    public function edit_index()
    {
        ...
        if (IS_POST) {
            ...
            $res = D('weiba_post')->where(array(
                'id' => $id
            ))->save($data);
            if ($res) {
                $this->success('编辑成功', addons_url('Weiba://Post/indexLists', array(
                    'mdm' => I('mdm')
                )));
            } else {
                $this->error('编辑失败！');
    ...
    public function post_data()
    {
        ...
        $list_data = M('weiba_post')->where($map)
            ->order('id DESC')
            ->page($page, 20)
            ->selectPage(20);
        foreach ($list_data['list_data'] as &$data) {
