...
class ShopVoteOptionModel extends Model
{
    ...
    // 为每个选项分配编号
    public function getNumber($voteId)
    {
        $map['vote_id'] = $voteId;
        $map['token'] = get_token();
        $number = $this->where($map)
    ...
    // 获取活动选项
    public function getOptions($voteId, $update = false)
    {
        // 以实时的opt_count来排序时，不能再用缓存
        $map['vote_id'] = $voteId;
        $map['token'] = get_token();
        $info = $this->where($map)
            ->order('opt_count desc,number asc')
            ->select();
        return $info;
    }
    
    // 通用的清缓存的方法
    public function clear($optIds, $voteId = 0)
    {
    ...
    // 获取用户投票信息
    public function getUserVoteLog($vote_id, $uid, $update = false)
    {
