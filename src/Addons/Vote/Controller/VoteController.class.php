...
class VoteController extends AddonsController
{
    ...
    public function edit()
    {
        ...
        if (IS_POST) {
            ...
            $option_list = M('vote_option')->where('vote_id=' . $id)
                ->order('`order` asc')
                ->select();
    ...
    public function checkPostData()
    {
        ...
        if (! I('post.start_date')) {
            ...
            $this->error('请选择结束时间');
        } elseif (strtotime(I('post.start_date')) >= strtotime(I('post.end_date'))) {
    ...
    public function _getVoteInfo($id)
    {
        // 检查ID是否合法
        if (empty($id) || 0 == $id) {
            $this->error("错误的投票ID");
    ...
    {
        $id = I('id');
        $url = addons_url('Vote://Vote/index', array(
            'id' => $id
        ));
        redirect($url);
    }
