...
class WeixinMessageController extends HomeController
{
    ...
    public function collect()
    {
        ...
        $list = M('weixin_message')->where($map)
            ->order('id desc')
            ->selectPage();
    ...
    public function _deal_content($data)
    {
        vendor("qqface");
...
;
    ...
    public function unicode_decode($uStr)
    {
        $pattern = '/([\w]+)|(\\\u([\w]{4}))/i';
    ...
    // 新增临时图片素材
    public function get_image_media_id($cover_id)
    {
    ...
    public function set_meterial()
    {
        ...
        if ($type == 'text' && $message['Content']) {
            ...
            $material = M('material_text')->where($map)
                ->field('id,is_use')
                ->find();
            if (! empty($material)) {
