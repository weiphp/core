...
class WeixinController extends HomeController
{
    ...
    private function reply($data, $weixin)
    {
        ...
        if (! isset($addons[$key])) {
            ...
            $map['type'] = 'click';
            $customMenu = M('custom_menu')->where($map)
                ->order('id desc')
                ->find();
            if (! empty($customMenu)) {
                if ($customMenu['addon']) {
        ...
        if (! isset($addons[$key])) {
            $like['keyword'] = $key;
            $like['keyword_type'] = 0;
            $keywordArr = M('keyword')->where($where)
                ->where($like)
                ->order('id desc')
                ->find();
    ...
    // 未发送成功的消息重新发
    public function sendOldMessage($token, $openid)
    {
