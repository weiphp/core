...
class PublicGroupController extends AdminController
{
    ...
    public function PublicGroup()
    {
        ...
        $public_list = M('public')->field('group_id,count(id) as public_count')
            ->group('group_id')
            ->select();
    ...
    // 通过插件ID转换成名字
    public function _idToName($ids, $list)
    {
        foreach ($ids as $id) {
            $res[] = $list[$id];
        }
    ...
    public function _deal_addon(&$info)
    {
        $addon_list = D('Home/Addons')->getWeixinList(true);
    ...
    public function PublicAdmin()
    {
        ...
        $this->assign('model', $model);
        $this->meta_title = $model['title'] . '管理';
        $this->assign('add_url', U('Home/Public/add', array(
            'from' => 5
        )));
        $this->display('Think:lists');
    }
