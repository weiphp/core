...
class UserController extends AdminController
{
    ...
    public function index()
    {
        ...
        $auth_group = M('AuthGroup')->where('status>0 and manager_id=0')
            ->field('id,title')
            ->select();
     ...
     * 显示用户公众号信息
     */
    public function showPublic($uid)
    {
        if (! empty($uid)) {
            $map['uid'] = $uid;
    ...
    // 修改审核
    public function changeAudit($is_audit, $uid)
    {
