...
class MaterialController extends HomeController
{
    ...
    public function material_lists()
    {
        ...
        $list = M('material_news')->where($map)
    ...
    public function get_news_by_group_id()
    {
        $map['group_id'] = I('group_id');
        // $map ['manager_id'] = $this->mid;
        $map['token'] = get_token();
    ...
    public function syc_news_to_wechat()
    {
        ...
        $child = M('material_news')->where($map2)
            ->field($field)
            ->select();
    ...
    public function _thumb_media_id($cover_id)
    {
    ...
    public function _image_media_id($cover_id)
    {
        $cover = get_cover($cover_id);
        $driver = C('PICTURE_UPLOAD_DRIVER');
        if ($driver != 'Local' && ! file_exists(SITE_PATH . $cover['path'])) { // 先把图片下载到本地
    ...
    public function picture_lists()
    {
        ...
        // $map ['manager_id'] = $this->mid;
        $map['token'] = get_token();
        $list = M('material_image')->where($map)
            ->field('id,cover_url')
            ->order('id desc')
            ->selectPage(39);
        $this->assign($list);
    ...
    public function picture_data()
    {
        ...
        $map['is_use'] = 1;
        $map['token'] = get_token();
        $data = M('material_image')->where($map)
            ->field('id,cover_url')
            ->order('id desc')
            ->select();
        $list['list_data'] = $data;
    ...
    // 根据id获取图片素材,设置欢迎语用到
    public function ajax_picture_by_id()
    {
        $id = I('image_id');
    ...
    // 根据id获取图片素材,设置欢迎语用到
    public function ajax_voice_by_id()
    {
        $id = I('voice_id');
    ...
    public function voice_lists()
    {
        ...
        $map['type'] = 1;
        $map['is_use'] = 1;
        $list = M('material_file')->where($map)
            ->order('id desc')
            ->selectPage(30);
    ...
    public function voice_data()
    {
        ...
        $map['token'] = get_token();
        $map['type'] = 1;
        $data = M('material_file')->where($map)
            ->order('id desc')
            ->select();
        $list['list_data'] = $data;
        vendor("getID3.getid3.getid3");
    ...
    // 根据id获取图片素材,设置欢迎语用到
    public function ajax_video_by_id()
    {
        $id = I('video_id');
