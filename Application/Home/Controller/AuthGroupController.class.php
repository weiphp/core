...
class AuthGroupController extends HomeController
{
 ...
 // 是否有创建微信带参数的二维码权限
    ...
    public function del($model = null, $ids = null)
    {
        ...
        if ($res) {
            $px = C('DB_PREFIX');
            $follow_list = M()->table($px . 'auth_group_access a')
                ->join($px . 'public_follow f ON a.uid=f.uid')
                ->where($map2)
    ...
    public function updateWechatGroup()
    {
        ...
        $map['manager_id'] = $this->mid;
        $map['type'] = 1;
        $group_list = M('auth_group')->where($map)
    ...
    public function tongbu_follow()
    {
        ...
        $map['has_subscribe'] = 1;
        $map['token'] = get_token();
        $follow_list = M('public_follow')->where($map)
            ->order('id asc')
            ->limit(5)
            ->select();
        if (! $follow_list) {
