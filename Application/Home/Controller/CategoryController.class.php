...
class CategoryController extends HomeController
{
    ...
    public function exportData()
    {
        ...
        $map['module'] = I('module');
        $list = M('common_category')->where($map)
            ->field('id,name,title,sort,pid')
            ->limit(10000)
            ->select();
        $arr[0] = $ht;
        if (! empty($list)) {
    ...
    public function inputData()
    {
        ...
        if (IS_POST) {
            ...
            for ($j = 2; $j <= $highestRow; $j ++) {
                $addData['code'] = $map['code'] = trim((string) $objPHPExcel->getActiveSheet()
                    ->getCell('A' . $j)
                    ->getValue());
                $addData['title'] = trim((string) $objPHPExcel->getActiveSheet()
                    ->getCell('B' . $j)
                    ->getValue());
                $addData['pid'] = trim((string) $objPHPExcel->getActiveSheet()
                    ->getCell('C' . $j)
                    ->getValue());
