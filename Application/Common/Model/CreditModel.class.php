...
class CreditModel extends Model
{
    ...
    // 通过积分标识获取积分配置值
    public function getCreditByName($credit_name = null)
    {
        $token = get_token();
    ...
    public function updateFollowTotalCredit($uid)
    {
        ...
        $map['token'] = get_token();
        $info = $this->where($map)
            ->field('sum(score) as score, sum(experience) as experience')
            ->find();
    ...
    public function updateSubscribeCredit($token, $credit, $type = 0)
    {
        if ($type == 0) {
