 ...
 // 用户名被占用
    ...
    public function getUserInfo($uid, $update = false)
    {
        ...
        if ($userInfo === false || $update) {
            ...
            // 公众号管理员信息
            $manager = (array) M('manager')->where("uid='$uid'")
                ->field(true)
                ->find();
            ...
            $prefix = C('DB_PREFIX');
            $groups = M()->table($prefix . 'auth_group_access a')
                ->where("a.uid='$uid' and g.status='1'")
                ->join($prefix . "auth_group g on a.group_id=g.id")
                ->field('a.group_id,g.title,g.type,g.icon')
                ->select();
            foreach ($groups as $g) {
            ...
            $tokens = M('public_follow')->where("uid='$uid'")
                ->field(true)
                ->select();
            foreach ($tokens as $t) {
    ...
    public function _deal_nickname($data, $type = 0)
    {
