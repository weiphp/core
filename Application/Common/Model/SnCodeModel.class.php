...
class SnCodeModel extends Model
{
    ...
    public function getInfoById($id, $field = '', $update = false, $data = array())
    {
    ...
    public function delayAdd($data = array(), $delay = 10)
    {
        if (empty($data)) {
            return true;
        }
    ...
    // 延时插入，与thinkphp的延时插入不同，它同时需要更新相关的缓存，保证数据的实时性
    public function delayAdd_bar($data = array(), $delay = 10)
    {
        ...
        if (empty($data)) {
            if (! empty($dataArr)) {
                ...
                foreach ($dataArr as $d) {
                    ...
                    (new \Addons\Coupon\Model\CouponModel())->updateCollectCount($d['target_id'], true);
                }
            }
            return true;
        }
    ...
    public function getCollectCount($target_id, $addon = 'Coupon', $update = false, $cache_update = false)
    {
    ...
    public function getMyList($uid, $target_id = '', $addon = 'Coupon', $update = false, $cache_id = '')
    {
    ...
    public function getMyAll($uid, $addon = 'Coupon', $update = false, $cache_id = '', $can_use = '')
    {
    ...
    public function update($id, $save = array())
    {
        $map['id'] = $id;
    ...
    public function getSnCount($type = '')
    {
        $count = 0;
        $mid = get_mid();
    ...
    public function getUserCount($uid)
    {
        ...
        $list = $this->where($map)
            ->field('count(can_use) as total,sum(can_use) as left_count, addon')
            ->group('addon')
            ->select();
        foreach ($list as $vo) {
