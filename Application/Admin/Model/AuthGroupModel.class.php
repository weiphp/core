...
class AuthGroupModel extends Model
{
 ...
 // 分类权限标识
    ...
    public function addToGroup($uid, $gid)
    {
        ...
        $uid_arr = explode(',', $uid);
        $uid_arr = array_diff((array) $uid_arr, array(
    ...
    public static function getUserGroup($uid)
    {
        ...
        $prefix = C('DB_PREFIX');
        $user_groups = M()->field('uid,group_id,title,description,rules')
    ...
    public static function getAuthExtend($uid, $type, $session)
    {
        ...
        $prefix = C('DB_PREFIX');
        $result = M()->table($prefix . self::AUTH_GROUP_ACCESS . ' g')
     ...
     * 批量设置用户组可管理的扩展权限数据
     *
     * @param int|string|array $gid
     *            用户组id
     * @param int|string|array $cid
     *            分类id
     *            
     * @author 朱亚杰 <xcoolcc@gmail.com>
     */
    ...
    {
        return M(self::AUTH_GROUP_ACCESS)->where(array(
            'uid' => $uid,
            'group_id' => $gid
        ))->delete();
    }
     ...
     * @author 朱亚杰 <zhuyajie@topthink.net>
     */
    public static function userInGroup($group_id)
    {
        $prefix = C('DB_PREFIX');
